...
class NewsList extends Component {
  ...
  columns = [
    ...
    },
    {
      ...
      dataIndex: 'title',
      width: 200,
      render: val => val.length > 20 ? `${ val.substring(0, 20)}...` : val
    },
    {
      title: '新闻简介',
      dataIndex: 'introduce',
      render: val => val.length > 30 ? `${ val.substring(0, 30)}...` : val
    },
    {
      title: '创建时间',
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          <a href="#" onClick={ () => this.handleUpdateModalVisible(true, record) }>编辑</a>&nbsp;
          <a href="#" onClick={ () => this.handlePublish(record) }>{ record.status == 0 ? '发布' : '已发布'}</a>&nbsp;
          <Popconfirm title="确定要删除吗？" okText="确定" cancelText="取消" onConfirm={ () => this.handleRemove(record) }>
            <a href="#">删除</a>
          </Popconfirm>
        </Fragment>
      ),
...
export default Form.create()(NewsList);
