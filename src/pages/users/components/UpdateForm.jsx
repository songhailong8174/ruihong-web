import { Form, Input, Button, Alert, Drawer, Icon, Descriptions, Badge } from 'antd';
  ...
  return (
    ...
    >
      <Descriptions title="企业信息" layout="vertical" bordered>
        ...
        <Descriptions.Item label="纳税人识别号">{values.taxno}</Descriptions.Item>
        <Descriptions.Item label="对公账号">{values.accountnum}</Descriptions.Item>
        ...
        <Descriptions.Item label="认证状态">{values.authstatus == 1 ? '已认证' : '未认证'}</Descriptions.Item>
        <Descriptions.Item label="营业执照">
          <img src={values.license} style={{
            width: '100%',
            height: 'auto'
          }} alt=""/>
        </Descriptions.Item>
      ...
      >
        <Button onClick={() => handleUpdateModalVisible()} style={{ marginRight: 8 }}>
          取消
        </Button>
